...
ckpt_manager = tf.train.CheckpointManager(ckpt,"./checkpoints/without_sentiment_inceptionV3", max_to_keep=5)


# In[31]:

# get the latest checkpoint from the path
status = ckpt.restore(tf.train.latest_checkpoint("./checkpoints/without_sentiment_inceptionV3"))
...
# Load the graph variables from the checkpoint
tf.train.list_variables(tf.train.latest_checkpoint("./checkpoints/without_sentiment_inceptionV3"))

# In[34]:
# captions on a random image from the validation set
...
# opening the image
print(int(img_name_val[rid].split('/')[-1].split('_')[-1].split('.')[0]))
...
# make result caption list for inceptionv3
result_caption_list = []
...
with open('results_without_sentiment_inceptionV3.json', 'w') as f:
...
cocoRes = coco.loadRes('results_without_sentiment_inceptionV3.json')
